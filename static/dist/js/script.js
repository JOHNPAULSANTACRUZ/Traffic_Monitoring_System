...
document.addEventListener('keydown', function(e) {
    if (e.key == 'l') {
        ...
        drawMode = "none"; // Set to 'none' to stop draw mode
        canvas.style.cursor = 'default'; // Set cursor back to default
        modeMessage.innerHTML = 'Draw Mode: None (press <kbd>l</kbd> to change to line drawing, <kbd>p</kbd> to change to polygon drawing)';
    }
});
...
// placeholder image
let img = new Image();
img.src = './static/dist/img/first_frames/first_frame.png';
img.onload = function() {
...
function writePoints(parentPoints) {
    ...
    var code_template = `${parentPoints.map(function(points) {
        ...
        return `[${point[0]}, ${point[1]}]`;}).join(',')}]`;}).join(',')}`;
    document.querySelector('#array').innerHTML = code_template;
}
